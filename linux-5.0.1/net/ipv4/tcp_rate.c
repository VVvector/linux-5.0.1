...
/*
 * 应用层限制：
 * 如下函数 tcp_rate_check_app_limited()，同时满足以下条件，则认为是应用程序受限（即应用层下发的数据不够快）
 * 1. 如果套接口发送缓存中的数据长度小于MSS值，即认为要发送的数据量小于一个packet大小。
 * 2. 本机队列，包括Qdisc队列/网卡发送队列，没有数据（小于数据长度为1的SKB所占用的空间）;
 * 3. 正在网络中的报文数量小于拥塞窗口（即认为并非拥塞窗口限制了报文发送）;
 * 4. 所有丢失报文都已经被重传;
 * 
 * 如果确认报文数量与网络中报文数量之和不为零，将结果赋予变量app_limited，否则，app_limited赋值为1。
