 ...
 * 1. 如果套接口发送缓存中的数据长度小于MSS值;
 * 2. 本机Qdisc队列/网卡发送队列中没有数据（小于数据长度为1的SKB所占用的空间）;
 * 3. 发送到网络中的报文数量小于拥塞窗口（并非拥塞窗口限制了报文发送）;
 * 4. 所有丢失报文都已经被重传;
 * 
 * 如果确认报文数量与网络中报文数量之和大于零，将结果赋予变量app_limited，否则，app_limited赋值为1。
