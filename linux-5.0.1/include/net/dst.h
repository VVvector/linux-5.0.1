...
/*
 * 最终生成的 IP 数据报的路由称为目的入口 (dst_entry)，目的入口反映了相邻的
 * 外部主机在本地主机内部的一种“映象”。它是与协议无关的目的路由缓存相关的数据
 * 结构，包括了路由缓存链接在一起的数据结构成员变量、垃圾回收相关的成员变量、邻
 * 居项相关的成员、二层缓存头相关的成员、输入/输出函数指针以用于命中路由缓存的数
 * 据包进行后续的数据处理等。
*/
struct dst_entry {
